...
async function scrapeEpisodeServers(episodeUrl) {
  ...
  const servers = [];
  const serverElems = $('.server, .player-option, .option, li[data-video], .play-box-iframe');
  ...
  serverElems.each((i, elem) => {
    try {
      ...
      let serverUrl = $elem.attr('data-video') || $elem.attr('data-src') || $elem.attr('data-url') || '';

      if (!serverUrl) {
        const iframe = $elem.find('iframe').first();
        if (iframe.length) {
          serverUrl = iframe.attr('src') || iframe.attr('data-src') || '';
        }
      }

      if (serverUrl) {
        const qualityElem = $elem.find('.quality, .qlty').first();
        const quality = qualityElem.length ? cleanText(qualityElem.text()) : null;

        servers.push({
          name,
          url: serverUrl.startsWith('http') ? serverUrl : makeAbsoluteUrl(serverUrl),
          quality,
        });
      }
    } catch (error) {
